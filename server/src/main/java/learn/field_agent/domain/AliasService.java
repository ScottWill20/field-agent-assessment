...
public class AliasService {
    ...
    private Result<Alias> validate(Alias alias) {
        ...
        for (Alias existingAlias : aliases) {
            if (alias.getName().equals(existingAlias.getName()) && Validations.isNullOrBlank(alias.getPersona())) {
                result.addMessage("persona is required for duplicate agent", ResultType.INVALID);
            }
        }
